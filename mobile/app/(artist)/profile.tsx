...
import {
  apiGetMyArtistProfile, apiGetMyReceivedComments, apiGetMyFollowers,
  ReceivedComment, ArtistProfile, Follower,
...
export default function ArtistProfileScreen() {
  ...
  const openFollowers = useCallback(async () => {
    ...
    try {
      const data = await apiGetMyFollowers();
      setFollowers(data);
    } catch {}
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <View style={styles.infoSection}>
          ...
          <TouchableOpacity style={styles.followerBtn} onPress={openFollowers} activeOpacity={0.7}>
            <Ionicons name="people-outline" size={14} color={Colors.primary} />
            <Text style={styles.followerCount}>
              <Text style={styles.followerNum}>{artistProfile?.follower_count ?? 0}</Text>
              {' '}粉丝
            </Text>
        ...
        <View style={styles.tabBar}>
          <TabBtn
            label={`作品集 (${works.length})`}
            active={activeTab === 'works'}
            onPress={() => setActiveTab('works')}
          />
          <TabBtn
            label="收到的评论"
            active={activeTab === 'received-comments'}
            onPress={() => setActiveTab('received-comments')}
          />
        </View>
      ...
      >
        <View style={[modal.root, { paddingTop: insets.top || 20 }]}>
          <View style={modal.header}>
            <Text style={modal.title}>粉丝列表</Text>
          ...
          {followersLoading ? (
            ...
            <FlatList
              data={followers}
              keyExtractor={(f) => f.id}
              contentContainerStyle={followers.length === 0 ? modal.emptyContainer : modal.list}
              ListEmptyComponent={
                <View style={modal.empty}>
                  <Ionicons name="people-outline" size={52} color={Colors.border} />
                  <Text style={modal.emptyText}>还没有粉丝</Text>
                </View>
              }
              renderItem={({ item }) => (
                <View style={modal.followerItem}>
                  <Avatar name={item.name} uri={item.avatar_url} size={44} />
                  <Text style={modal.followerName}>{item.name}</Text>
                </View>
              )}
            />
          )}
...
const styles = StyleSheet.create({
  ...
  infoSection: { alignItems: 'center', paddingTop: 12, paddingBottom: 14, backgroundColor: '#F2F2F7' },
  ...
  ratingBadge: { backgroundColor: '#FFFBEB', paddingHorizontal: 9, paddingVertical: 3, borderRadius: 20 },
  ratingText: { fontSize: 12, color: '#D97706', fontWeight: '700' },
  ipBadge: {
  ...
  followerBtn: {
    ...
    backgroundColor: '#FFF0F5', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20,
  },
  followerCount: { fontSize: 13, color: Colors.primary },
  followerNum: { fontWeight: '700' },
  tabBar: {
    flexDirection: 'row', backgroundColor: '#fff', marginHorizontal: 16, marginTop: 2,
...
const modal = StyleSheet.create({
  ...
  followerName: { fontSize: 15, fontWeight: '600', color: Colors.text },
});
